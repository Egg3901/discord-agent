import type { TextChannel, ThreadChannel, DMChannel, Message } from 'discord.js';
...
export type SessionChannel = ThreadChannel | DMChannel | TextChannel;
...
/**
 * Core per-message processing: add the user message to the session, run the
 * appropriate streaming / agent loop, stream output back, and send the
 * completion / next-step UI. Shared by the messageCreate handler and by the
 * next-step button handler so that button follow-ups drive the same pipeline
 * without relying on Discord's messageCreate event (which filters bot-authored
 * messages and therefore silently dropped button-triggered prompts).
 */
export async function processSessionMessage(
  ...
  const imageAttachments = opts.imageAttachments;

  if (session.busy) {
    await channel
      .send('Still working on the previous message. Please wait for it to finish (or react with 🛑 to cancel).')
      .catch(() => {});
    return;
  }

  try {
    await (channel as TextChannel | ThreadChannel | DMChannel).sendTyping().catch(() => {});
      ...
      if (isCC) {
        ...
        if (toolCallCount > 0) {
          await sendCompletionWithNextSteps(
            ...
            {
              onFollowUp: (prompt) =>
                processSessionMessage(channel, session, userId, prompt, deps),
            },
          );
        }
      } else if (hasTools) {

...
export class KeyStore {
  ...
  save(id: string, apiKey: string, provider: Provider, addedBy: string = 'system'): void {
    const db = getDatabase();
    db.prepare(
      'INSERT OR REPLACE INTO api_keys (id, api_key, provider, added_at, added_by, total_requests, is_active) VALUES (?, ?, ?, ?, ?, 0, 1)',
    ).run(id, apiKey, provider, Date.now(), addedBy);
  }
